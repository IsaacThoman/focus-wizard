...
async function captureScreenBitmap(): Promise<{ bitmap: Buffer; width: number; height: number }> {
  ...
  const thumbHeight = Math.max(1, Math.round(primaryDisplay.size.height * scale))
